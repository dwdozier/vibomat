...
To use the Spotify Playlist Builder, you must first register an application in the Spotify Developer Dashboard. This will provide you with the necessary credentials (`Client ID` and `Client Secret`) to authenticate with the Spotify API.
...
2.  **Create a New App**
    ...
    - **Redirect URI**: This is a critical step for OAuth authentication. You **must** add the following URI:
      ```
...
Copy your **Client ID** and **Client Secret**. You will need these to configure the Playlist Builder. Return to [SETUP.md](SETUP.md) to continue the installation process.
